...
async def echo(context, event_queue) -> None:
    ...
    await tu.start_work()
    await tu.add_artifact([ExtendedPart.from_text(f"echo: {context.get_user_input()}")])
    await tu.complete()
...
async def test_send_parts_returns_typed_parts(make_app):
    async def mixed(context, event_queue):
        tu = ExtendedTaskUpdater(context, event_queue)
        await tu.start_work()
...
async def test_streaming_parts_reach_the_emit_callback(make_app):
    async def streaming(context, event_queue):
        tu = ExtendedTaskUpdater(context, event_queue)
        await tu.start_work()
        emit = tu.as_part_emitter()
        await emit(ExtendedPart.thinking("step one"))
        await emit(ExtendedPart.thinking("step two"))
        await tu.add_artifact([ExtendedPart.from_text("done")])
        await tu.complete()
...
async def test_a_failing_agent_raises_with_the_state_that_caused_it(make_app):
    async def broken(context, event_queue):
